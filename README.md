...
검사 선택·문진 → 카메라 촬영 또는 업로드 → 눈 영역 추출 → 5개 클래스 분류와
Grad-CAM → 결과 확인·이력/보고서가 기본 흐름입니다. 일반 LLM 설명은 명시적으로
선택한 provider가 준비됐을 때만 사용하며, MedGemma 연구 VLM은 운영 결과와 분리됩니다.
구현·검증 근거와 포트폴리오용 화면 목록은 [PORTFOLIO](docs/PORTFOLIO.md)에 있습니다.

> 이 시스템은 의료 진단을 확정하는 장비가 아니라 이상 징후 선별과 설명을 돕는 연구용 보조 도구입니다. 최종 판단은 의료 전문가의 진료로 확인해야 합니다.

## 현재 구현 기준

이 문서는 2026-09-24 로컬 `main`의 `9b9625a` 소스 경로를 기준으로 점검했습니다.
아래 장비 상태는 [2026-09-23 Admin Control 기록](docs/agent/admin-control/HANDOFF.md)의
마지막 관측으로, 이번 문서 작업에서 장비를 다시 조회한 결과가 아닙니다. A의 마지막
기록된 배포는 `9cc410a`, B controller release는 `86bebb6`이며 저장소 `main`과
동일한 배포 SHA라고 가정하지 않습니다.

| 영역 | 현재 소스·기록에 근거한 범위 | 남은 확인 |
| --- | --- | --- |
| 기본 안구 | MediaPipe Face Mesh, EfficientNet-B0 5개 클래스, Grad-CAM | 의료 정확도 재현 평가 |
| 피부·두피 | UI와 등록 구조, 모델 `not_configured` | 모델·평가·운영 검증 |
| 결과 설명 | OpenAI/Gemini/명시적 local provider 중 하나를 선택 | provider 준비·응답 품질 |
| 연구 VLM | E0~E4를 운영 결과와 분리; 합성 입력 경로 기록 | 라벨 있는 임상 평가 |
| Admin Control | 9월 23일 로그인·overview·동일 설정 draft→plan 관측 | 새 live Apply·모델 전환은 미실행 |
...
이 목록의 UI·관리 기능 구현은 모든 옵션이 상시 켜져 있다는 뜻이 아닙니다.
특히 로컬 LLM 실패가 클라우드로 자동 전환되지는 않습니다. 합성 입력 기반
E0~E4 통합 성공은 질환 정확도나 임상 유효성을 증명하지 않습니다.
...
아래는 개발·운영 환경에서 선택해 실행할 수 있는 명령입니다. 이번 문서 점검의
실행 결과나 의료 성능 수치가 아닙니다.
